...
response::response(std::string reponse_string)
    : content(reponse_string), status_code(200) {}
response::response(std::string &path, std::string &type)
    : path(path), type(type) {
  if (path == "rat") {
    status_code = 405;
...
response::response(location_info local_info,
                   std::map<int, std::string> error_page, std::string &path)
  ...
  if (local_info.client_max_body_size != 0 &&
      (unsigned int)local_info.len > local_info.client_max_body_size) {
...
listFilesRecursively(const char *basePath, std::vector<std::string> &lst,
...
std::string list_directory(std::string &content_type, std::string &content,
                           int &content_length, location_info local_info,
                           std::map<std::string, location_info> &lst_info) {
  (void)content_length;
  ...
  lst_info[local_info.root.substr(1, local_info.root.length())].root =
      local_info.root;
  lst_info[local_info.root.substr(1, local_info.root.length())].autoindex =
      true;
  std::vector<std::string> lst;
  std::vector<std::string> files =
      listFilesRecursively(local_info.root.c_str(), lst, lst_info);
  ...
  if (files.size() != 0) {
    for (size_t val = 0; val != files.size(); ++val) {
      ...
      content += "<tr><td><a href=\"" + files[val] + "\">" +
                 files[val].substr(files[val].find_last_of("/")) +
                 "</a></td><td>" + last_modified + "</td><td>";
    }
    content += "<tr><td><a href=\"/" + lst_info[files[0]].index +
               "\">.</a></td><td></td></tr>";
    content += "<tr><td><a href=\"/" +
               lst_info[files[0]].index.substr(
                   0, lst_info[files[0]].index.find_last_of("/")) +
               "\">..</a></td><td></td></tr>";
  }
...
std::string
response::build_response(std::map<std::string, location_info> &lst_info) {
  std::string content;
  time_t rawtime;
  struct tm *timeinfo;
  char time_string[80];

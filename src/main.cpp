...
void servers_starter(std::vector<server_info> lst_config,
                     std::vector<server> &lst_servers) {
  for (std::vector<server_info>::iterator it = lst_config.begin();
       it != lst_config.end(); it++)
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    for (std::vector<server_info>::iterator it = servers_info.begin();
         it != servers_info.end(); ++it) {
      std::cout << "server_info: " << it->port << std::endl;
    }
    servers_starter(servers_info, lst_server);
    while (1) {
      for (std::vector<server>::iterator it = lst_server.begin();
           it != lst_server.end(); it++) {
        it->run();
      }
    }

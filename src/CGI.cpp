...
std::string CGI::getExecPath() {
  std::string scriptExt =
      scriptName.substr(scriptName.find('.'), std::string::npos);
  std::map<std::string, std::string>::iterator cgi_it;
  if ((cgi_it = serverInfo.locations["/"].cgi.find(scriptExt)) !=
      serverInfo.locations["/"].cgi.end())
...
void CGI::setEnvVars() {
  ...
  size_t i = 0;
  size_t len;
  for (std::vector<std::string>::iterator it = vars.begin(); it != vars.end();
       it++, i++) {
    len = it->length() + 1;
    envp[i] = new char[len];

...
parser::parser(std::string _path) { parsefile(_path); }
...
void parser::check_errors(void) {
  ...
  while (it != ext_file.end()) {
    ...
    if (*it == "server" ||
        ((*it).find("location") == 0 && (*it).find('/') != std::string::npos)) {
      if (*(++it) != "{")
        throw Exceptions::ConfigError();
      state = BRACE;
      bracket_state++;
...
void parser::manage_locations(std::vector<std::string>::iterator it) {
  ...
  servers.back().locations.insert(
      std::pair<std::string, location_info>(location, location_info()));
  it += 2;
  while (*it != "}") {
    ...
    if (field == "root") {
      ...
      servers.back().locations[location].cgi.insert(
          std::pair<std::string, std::string>(cgi, root));
      ...
      servers.back().locations[location].autoindex =
          ((data == "on") ? true : false);
    } else if (field == "upload_dir") {
      servers.back().locations[location].upload_dir = data;
    } else if (field == "allow_request") {
      while (data.length()) {
        servers.back().locations[location].allowed_requests.push_back(
            data.substr(0, data.find_first_of(WHITESPACES)));
        if (data.find_first_of(WHITESPACES) == std::string::npos)
          data = "";
        else {
          data =
              data.substr(data.find_first_of(WHITESPACES), std::string::npos);
          data = trim(data);
        }
      }
      ...
      servers.back().locations[location].client_max_body_size =
          atoi(data.c_str());
  ...
  while (++it != ext_file.end()) {
    if (*it == "server")
      ...
      if (field == "listen") {
        ...
        servers.back().error_pages.insert(std::pair<int, std::string>(
            atoi(data.c_str()), data.substr(data.find_last_of(WHITESPACES) + 1,
                                            std::string::npos)));
      } else
        throw Exceptions::UnknownFieldError();
    }
...
void parser::set_max_body_size() {
  ...
  for (; it != servers.end(); it++) {
    std::map<std::string, location_info>::iterator it_loc =
        it->locations.begin();
    for (; it_loc != it->locations.end(); it_loc++) {
      if (it_loc->second.client_max_body_size == 0 &&
